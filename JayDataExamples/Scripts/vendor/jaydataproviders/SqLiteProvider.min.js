...
h},this),i="T0";e.length>0&&(i="T"+e[0].AliasNumber);g.Association.associationInfo.ReferentialConstraint.forEach(function(a){b.addText(i+"."+a[g.Association.associationInfo.From]);b.addText(" = ");b.addText(f+"."+a[g.Association.associationInfo.To])},this);b.addText(")")}},this)},VisitDefaultProjection:function(a){a.selectTextPart("projection");var b=this.infos.filter(function(a){return a.IsMapped}).length>1;if(a.sets.length>1){a.addText(SqlStatementBlocks.select);a.sets.forEach(function(c,d){if(this.infos[d].IsMapped){var e=
a.getExpressionAlias(c);c.storageModel.PhysicalType.memberDefinitions.getPublicMappedProperties().forEach(function(c,f){(f>0||d>0)&&a.addText(SqlStatementBlocks.valueSeparator);a.addText(e+".");a.addText(c.name);if(b){a.addText(SqlStatementBlocks.as);a.addText(e+"__"+c.name)}},this)}},this)}else a.addText("SELECT *")}});$data.Expressions.ExpressionNode.prototype.monitor=function(a,b){return $data.sqLite.SqlExpressionMonitor.create(a).Visit(this,b)};
...
return c}},b),d=$data.sqLite.SqlCompiler.create(c,b);d.compile();$data.sqLite.SqlBuilder.create(this.sets,this.entityContext);a.modelBinderConfig={};$data.sqLite.sqLite_ModelBinderCompiler.create(a,b).Visit(c);return{sqlText:d.filters[0].selectedFragment.text,params:d.filters[0].selectedFragment.params,modelBinderConfig:a.modelBinderConfig}}},null);
$C("$data.sqLite.SqlPagingCompiler",$data.Expressions.EntityExpressionVisitor,null,{constructor:function(a){this.provider=a},compile:function(a,b){this.Visit(a,b)},VisitPagingExpression:function(a,b){this.Visit(a.amount,b)},VisitConstantExpression:function(a,b){b.addParameter(a.value);b.addText(SqlStatementBlocks.parameter)}});
...
VisitForEachExpression:function(a){this._defaultModelBinder(a)},VisitCountExpression:function(){var a=Container.createqueryBuilder();a.modelBinderConfig.$type=$data.Array;a.selectModelBinderProperty("$item");a.modelBinderConfig.$type=$data.Integer;a.modelBinderConfig.$source="cnt";a.resetModelBinderProperty();this._query.modelBinderConfig=a.modelBinderConfig},VisitExpression:function(a,b){var c=Container.createFindProjectionVisitor();c.Visit(a);c.projectionExpression?this.Visit(c.projectionExpression,
b):this.DefaultSelection(b)},_defaultModelBinder:function(a){var b=Container.createqueryBuilder();b.modelBinderConfig.$type=$data.Array;b.modelBinderConfig.$item={};b.selectModelBinderProperty("$item");this.VisitExpression(a,b);b.resetModelBinderProperty();this._query.modelBinderConfig=b.modelBinderConfig},_addPropertyToModelBinderConfig:function(a,b){var c=this._query.context._storageModel.getStorageModel(a);a.memberDefinitions.getPublicMappedProperties().forEach(function(a){if(!c||c&&!c.Associations[a.name]&&
!c.ComplexTypes[a.name]){a.key&&(this.currentObjectFieldName?b.addKeyField(this.currentObjectFieldName+"__"+a.name):b.addKeyField(a.name));b.modelBinderConfig[a.name]=this.currentObjectFieldName?this.currentObjectFieldName+"__"+a.name:a.name}},this);c&&this._addComplexTypeProperties(c.ComplexTypes,b)},_addComplexTypeProperties:function(a,b){a.forEach(function(a){b.selectModelBinderProperty(a.FromPropertyName);b.modelBinderConfig.$type=a.ToType;var d=this.currentObjectFieldName;this.currentObjectFieldName=
this.currentObjectFieldName?this.currentObjectFieldName+"__":"";this.currentObjectFieldName=this.currentObjectFieldName+a.FromPropertyName;this._addPropertyToModelBinderConfig(a.ToType,b);b.popModelBinderProperty();this.currentObjectFieldName=d},this)},DefaultSelection:function(a){a.modelBinderConfig.$type=this._query.defaultType;this._query.context._storageModel.getStorageModel(this._query.defaultType);if(this.sqlContext.infos.filter(function(a){return a.IsMapped}).length>1)this.currentObjectFieldName=
this._sqlBuilder.getExpressionAlias(this.sqlContext.sets[0]);this._addPropertyToModelBinderConfig(this._query.defaultType,a);this.sqlContext.infos.forEach(function(b,c){if(c>0&&b.IsMapped){var d=b.NavigationPath.split(".");d.shift();d.forEach(function(e,f){if(e)if(a.modelBinderConfig[e])a.selectModelBinderProperty(e);else{a.selectModelBinderProperty(e);var h=false;if(b.Association.associationInfo.ToMultiplicity==="*"&&d.length-1===f){a.modelBinderConfig.$type=$data.Array;a.selectModelBinderProperty("$item");
h=true}a.modelBinderConfig.$type=this.sqlContext.sets[c].elementType;this.currentObjectFieldName=this._sqlBuilder.getExpressionAlias(this.sqlContext.sets[c]);this._addPropertyToModelBinderConfig(this.sqlContext.sets[c].elementType,a);h&&a.popModelBinderProperty()}},this);for(var e=0;e<d.length;e++)a.popModelBinderProperty()}},this)},VisitProjectionExpression:function(a,b){this.hasProjection=true;this.Visit(a.selector,b);a.selector&&a.selector.expression instanceof $data.Expressions.ObjectLiteralExpression&&
