...
(this.providerConfiguration.databaseName=this.providerConfiguration.localStoreName)},initializeStore:function(a){var a=$data.typeSystem.createCallbackSetting(a),b=[],c;for(c in this.context._entitySetReferences)b.push(this.context._entitySetReferences[c].collectionName);var e=null;if(this.providerConfiguration.persistentData&&window.localStorage&&this.providerConfiguration.dbCreation!==$data.storageProviders.DbCreationType.DropAllExistingTables){var d=this,f=window.localStorage.getItem(this.providerConfiguration.databaseName||
"JayData_InMemory_Provider");Object.isNullOrUndefined(f)||(e=JSON.parse(f,function(a,c){return-1<b.indexOf(a)&&c.map?c.map(function(b){return new d.context[a].createNew(b)}):c}))}f=e||this.dataSource||{};this.dataSource={inmemory_sequence:{}};for(var h=0;h<this.context._storageModel.length;h++){var g=this.context._storageModel[h];this.dataSource[g.TableName]=[];var i=g.LogicalType.memberDefinitions.getKeyProperties().filter(function(a){return a.computed});1<i.length&&Guard.raise(new Exception("More than one computed field not supported in "+
g.TableName+" entity set."));var k=!1;1===i.length&&(c=Container.resolveName(i[0].type),!0===this.supportedAutoincrementKeys[c]?(this.dataSource.inmemory_sequence[g.TableName]=0,k=!0):"function"!==typeof this.supportedAutoincrementKeys[c]&&console.log("WARRNING! '"+c+"' not supported as computed Key!"));if(f[g.TableName])for(c=0;c<f[g.TableName].length;c++){var j=f[g.TableName][c];j instanceof g.LogicalType||(e?j=new g.LogicalType(j):Guard.raise(new Exception("Invalid element in source: "+g.TableName)));
if(k){var l=j[i[0].name];l>this.dataSource.inmemory_sequence[g.TableName]&&(this.dataSource.inmemory_sequence[g.TableName]=l)}this.dataSource[g.TableName].push(j)}}a.success(this.context)},executeQuery:function(a,b){var b=$data.typeSystem.createCallbackSetting(b),c;try{c=this._compile(a)}catch(e){b.error(e);return}var d=[].concat(this.dataSource[a.context.getEntitySetFromElementType(a.defaultType).tableName]||[]);c.$filter&&!c.$every&&(d=d.filter(c.$filter));c.$map&&0===Object.keys(a.modelBinderConfig).length&&
(d=d.map(c.$map));if(c.$order&&0<c.$order.length){c.$order.reverse();for(var f=0,h=c.$order.length;f<h;f++)c.$order[f].ASC?d.sort(function(a,b){var d=c.$order[f](a),e=c.$order[f](b);return d===e?0:d>e?1:-1}):d.sort(function(a,b){var d=c.$order[f](a),e=c.$order[f](b);return d===e?0:d<e?1:-1})}void 0!==c.$take&&void 0!==c.$skip?d=d.slice(c.$skip,c.$skip+c.$take):void 0!==c.$take&&d.length>c.$take?d=d.slice(0,c.$take):c.$skip&&(d=d.slice(c.$skip,d.length));c.$some&&(d=[0<d.length]);c.$length&&(d=[d.length]);
a.rawDataList=d;b.success(a)},_compile:function(a){return(new $data.storageProviders.InMemory.InMemoryCompiler(this)).compile(a)},saveChanges:function(a,b){for(var c=0;c<b.length;c++){var e=b[c];switch(e.data.entityState){case $data.EntityState.Added:this._save_add_processPk(e);this.dataSource[e.entitySet.tableName].push(e.data);break;case $data.EntityState.Deleted:var d=this.dataSource[e.entitySet.tableName],f=this._save_getEntity(e,d),e=d.indexOf(f);d.splice(e,1);break;case $data.EntityState.Modified:if(e.data.changedProperties&&
0<e.data.changedProperties.length){d=this.dataSource[e.entitySet.tableName];f=this._save_getEntity(e,d);for(d=0;d<e.data.changedProperties.length;d++){var h=e.data.changedProperties[d];!h.key&&-1<e.entitySet.elementType.memberDefinitions.getPublicMappedPropertyNames().indexOf(h.name)&&(f[h.name]=e.data[h.name])}}}}if(this.providerConfiguration.persistentData&&window.localStorage){var e=this.providerConfiguration.databaseName||"JayData_InMemory_Provider",g=this,i=[];for(c in this.context._entitySetReferences)i.push(this.context._entitySetReferences[c].collectionName);
localStorageData=window.localStorage.setItem(e,JSON.stringify(this.dataSource,function(a,b){if(-1<i.indexOf(a)&&Array.isArray(b)){for(var c=[],d=0;d<b.length;d++){var e={};g.context[a].elementType.memberDefinitions.getPublicMappedProperties().forEach(function(a){if(!a.inverseProperty){var c=Container.resolveName(a.type),c=g.fieldConverter.fromDb[c];e[a.name]=c?c(b[d][a.name]):b[d][a.name]}});c.push(e)}return c}return b}))}a.success()},_save_add_processPk:function(a){var b=a.entitySet.elementType.memberDefinitions.getKeyProperties();
if(1===b.length&&b[0].computed){var b=b[0],c=Container.resolveName(b.type);"function"===typeof this.supportedAutoincrementKeys[c]?a.data[b.name]=this.supportedAutoincrementKeys[c]():!0===this.supportedAutoincrementKeys[c]&&(c=this.dataSource.inmemory_sequence[a.entitySet.tableName],a.data[b.name]=c+1,this.dataSource.inmemory_sequence[a.entitySet.tableName]=c+1)}else for(c=0;c<b.length;c++)(null===a.data[b[c].name]||void 0===a.data[b[c].name])&&Guard.raise(new Exception("Key field must set value! Key field name without value: "+
